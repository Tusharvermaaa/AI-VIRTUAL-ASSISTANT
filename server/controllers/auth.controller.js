...
export async function signup_auth(req, res) {
  ...
  if (user)
    return res.json({
      status: "user already exists in the system  auth controller ",
    });
  if (password.length < 3)
    return res.json({ status: "password length less then 3 auth controller" });
...
export async function logout_auth(req, res) {
  try {
    ...
    return res
      .status(200)
      .send({ message: "logout successfully from auth.controller" });
  } catch (err) {
