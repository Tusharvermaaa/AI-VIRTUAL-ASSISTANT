...
function Customize2() {
  ...
  const {selectedname ,setselectedname , selectedimg , userdata, setuserdata , serverurl   } = useContext(userdatacontext);
  const uploaddetails=async (e)=>{
    e.preventDefault();
    console.log("it is calling perfectly");
    const updateduser=await axios.post(`${serverurl}/current` , {selectedname , selectedimg},{withCredentials:true});
